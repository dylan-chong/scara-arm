...
typedef struct {
  int n_seg;
  ArmState states[100000]; // ugly, is it not?
} Job;
...
void set_motors(int in1,int in2, int in3)
{
 ...
 // gettimeofday(&sarm.time1,NULL);
  ...
  if( (in2<1000) || (in2>2000)){
     printf(" pwm2 out of range\n");
     return;
  }
  if( (in3<1000) || (in3>2000)){
     printf(" pwm3 out of range\n");
     return;
  }
  set_servo(0, in1);
  set_servo(1, in2); 
...
void calibrate_motors(){
  int m1;
  int m2 =1500; 
  printf("***MEASURE MOTOR 1\n");
  for (m1 = 1400; m1<1700;m1=m1+10){

     set_motors(m1, m2, 1500);
     take_picture();
     measure_angles(0);
     draw_motors();
     update_screen();
     printf(" pwm1=%d pwm2=%d angles: theta1=%f theta2=%f\n", 
               m1,m2,sarm.theta1, sarm.theta2);
     Sleep(0,100000);  
  }
  m1=1500; 
  printf("***MEASURE MOTOR 2\n");
  for (m2 = 1300; m2<1600;m2=m2+10){
...
int main()
{
   ...
   {   printf("Input: \n s-set motors \n m-measure angles \n f-read file \n e-execute job \n p-take and save picture \n i-print arm configuration \n o-edit arm parameters \n c-calibrate motors \n q-out?\n");
      ...
      if (input == 'f') // load file
       ...
       if (input == 'v') //run video
       {
          show_video(100);
       }
       if (input == 'c') //calibate motors
       {
          calibrate_motors();
       }
