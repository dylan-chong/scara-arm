...
int read_draw_file()
{
   ...
   int x = 0;
   int y = 0;
   unsigned char pen_pos = 'd';
   while (fscanf(file,"%s",in_str) != EOF) 
   {
     printf("in_str = %s   ", in_str);
    
     sscanf(in_str,"%d,%d,%c",&x,&y,&pen_pos);
...
int main()
{
  ...
  // int dt = 200000;
   ...
   while(input !='q')
   {
     ...
     update_screen();
     // single arcs
     if ( input == 'a') motors_pulse(0, volt,dt);
     if ( input == 's') motors_pulse(0,- volt,dt);
     if ( input == 'd') motors_pulse( volt,0,dt);
     if ( input == 'f') motors_pulse(- volt,0,dt);

     if ( input == 'z') motors_pulse(- volt,- volt,dt);
     if ( input == 'x') motors_pulse(- volt, volt,dt);
     if ( input == 'c') motors_pulse( volt,- volt,dt);
     if ( input == 'v') motors_pulse( volt, volt,dt);
